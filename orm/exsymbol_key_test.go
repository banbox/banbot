...
func TestGetExSymbol2PrefersCanonicalCacheEntry(t *testing.T) {
	...
	defer swapDefaultSymbolState(oldState)
	...
	if got := GetExSymbol2("macro", "macro", "CPI_US"); got == nil || got == base || got.ID != base.ID {
		t.Fatalf("expected empty exg_real symbol to become canonical, got %+v", got)
	}
	if got := GetExSymbol2("macro", "macro", "CPI_US", "fred"); got == nil || got == base || got.ID != base.ID {

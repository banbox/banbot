...
Web UI 使用独立的开发服务器参数；执行 `bot web -help` 查看当前的监听地址、端口和资源路径选项。
...
**bot kline agg**
按指定 `-pairs` 和 `-timeframes` 聚合大周期 K 线。
...
**bot kline verify**
校验 K 线数据与覆盖范围元数据是否一致；可使用 `-pairs`、`-tables` 和 `-batch-size` 缩小检查范围。

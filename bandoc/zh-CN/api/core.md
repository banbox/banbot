...
任务级可变核心状态由 `core.State` 表示，并由 `runtime.Runtime` 在创建任务时持有。它包含运行模式和环境、交易对与市场状态、订单簿、任务性能以及该任务自己的取消信号。新组件应通过 Runtime 注入的具体状态或依赖投影访问这些数据，而不是读写包级变量。

`Setup`、`SetRunMode`、`SetRunEnv`、`Sleep` 等包级接口仍为旧的嵌入式调用保留；它们属于兼容路径，不应用于需要与其他任务并行的新增运行流程。

...
当前交易器通过 `biz.RuntimeDeps` 接收一个 Runtime 的核心状态、时钟、市场、策略、订单、钱包、配置、存储与交易所依赖；`NewTraderWithRuntimeDeps` 不会回退查找缺失状态。订单处理和钱包等可变业务状态因此归属创建它们的任务。

`SetupComs` 与不带 RuntimeDeps 的旧构造路径仍用于兼容，不能用于需要和其他任务并行的新增执行流程。

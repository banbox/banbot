...
**字段语义澄清：** banbotraw 的 `info` 字段是旧版单字段复用设计。新方案在 TimescaleDB 中也统一使用拆分后的字段，并允许 K 线扩展列通过通用列级规则聚合，不再保留 `info` 或 `InfoBy()` 特判。
...
**`refreshAgg`（聚合更新）的影响：** banbotraw 中 `refreshAgg` 使用 PostgreSQL 的 `GROUP BY` 内联聚合 SQL（`INSERT INTO … SELECT … GROUP BY`）来高效更新大周期表。新方案中：
- TimescaleDB 路径：仍可使用 `INSERT INTO … SELECT … GROUP BY` 内联聚合，内置 K 线列按固定语义聚合：`quote`、`buy_volume`、`trade_num` 均为 `sum`
...
`queryUnfinish`、`SetUnfinish`、`DelKLineUn` 等函数通过 `IsQuestDB` 分支路由到对应实现。`getUnFinish` 的上层逻辑（`calcUnfinishFromSubs`、过期判断、`BuyVolume` 累加）完全不需要修改，因为两套路径返回的 `*banexg.Kline` 结构体字段完全一致（`Quote`、`BuyVolume`、`TradeNum` 均已填充）。
...
7. **聚合层**：`refreshAgg` 加 pg 路径（TimescaleDB 使用内联 `INSERT … SELECT … GROUP BY` 聚合，内置 K 线扩展列固定累加）
...
2. **`refreshAgg` 的 TimescaleDB 优化路径**：banbotraw 中 `refreshAgg` 使用 PostgreSQL 内联 `INSERT INTO … SELECT … GROUP BY` 聚合，效率高于 QuestDB 的应用层聚合。新方案中 TimescaleDB 路径可恢复此优化，聚合表达式为：`SUM(quote) AS quote`、`SUM(buy_volume) AS buy_volume`、`SUM(trade_num) AS trade_num`。
